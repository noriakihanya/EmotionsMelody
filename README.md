...
"la+n"を入力することで、n個分のコード進行がランダムで表示される。現在１〜１０まで対応（９は対象なし）

なおn個分のコード進行が得られないことがある（コードが１つや２つしか出力されない時がある）ため、任意のコード進行が得られるまで実行ボタンを押下して任意のコード数を得る。※対応方法については検討中
...
* 感情分析をしているが、必ずしも感情に沿ったメロディが生成されているとは言い難い。
